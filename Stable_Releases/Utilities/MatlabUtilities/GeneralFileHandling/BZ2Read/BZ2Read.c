 ...
 *    bzfid = BZ2Read('setpos', bzfid, uint32(new_position), [suppress_eof_warning])
 *                          (suppress_eof_warning is optional bool argument, default is false)
 * To read compressed data from current position:
 *    [bzfid data] = BZ2Read('read', bzfid, uint32(num_bytes_to_read), 'datatype', swapbytes)
 * To close file
...
bool mexBZ2File_open(mxArray*** p_plhs, const mxArray *prhs[]) {
    ...
    if (rawfile==NULL) {
        mexWarnMsgTxt("cold not open file in BZ2Read, no action taken");
        return false;
    }
...
bool mexBZ2File_setpos(mxArray*** p_plhs, const mxArray *prhs[], bool suppress_eof_warning) {
...
bool mexBZ2File_read(mxArray*** p_plhs, const mxArray *prhs[]) {
    ...
    *bzerror = (int) bzfid[2];
    ...
    bytes_read = BZ2_bzRead(bzerror, bzippedfile, mxGetData((*p_plhs)[1]), numbytes);
    currentpos = currentpos + bytes_read;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs<1 || !mxIsChar(prhs[0]))
        ...
        if (!((nlhs==1 && strcmp(op_mode,"read")!=0) || (nlhs==2 && strcmp(op_mode,"read")==0)))
            mexWarnMsgTxt("BZ2Read must return exactly one or two arguments, namely the information on the currently opened file, and the requested data in 'read' mode, no action taken");
        else {
            if (strcmp(op_mode,"open")==0) {
                ...
                else if (mexBZ2File_setpos(&plhs,prhs,suppress_eof_warning))
                    return;
            }
