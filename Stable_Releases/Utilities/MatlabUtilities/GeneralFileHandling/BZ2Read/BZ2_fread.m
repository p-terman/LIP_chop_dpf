% function [data count_read] = BZ2_fread(BZ2_fid, count, precision, skip, machineformat);
%
% This function interfaces with the BZ2Read function, and handles exactly
...
% the format specified at BZ2_fopen is used).
%
...
% for zipped files.  This is because in BZ2Read, the total space is
% allocated before reading begins, which requires finding the filesize
% before calling BZ2Read.  The filesize is found be running BZ2_filesize,
...
if nargin<2
    disp('Warning -- as apposed to fread, in BZ2_fread if you don`t specify count, a single character is returned.');
    count = 1;
end;
...
if ~(isnumeric(BZ2_fid) && length(BZ2_fid)==1 && BZ2_fid==fix(BZ2_fid) && BZ2_fid>0)
    disp('The first input to BZ2_fread should be a single BZ2_fid (positive integer)');
    return;
end;

if ~(isnumeric(count) && length(count)<=2 && all(count==fix(count)) && all(count >= 0))
    disp('The second input (count) to BZ2_fread should be a positive integer, or a vector of two positive integers, or inf');
    return;
end;

if ~ischar(precision)
    disp('The third argument to BZ2_fread should be a precision string.');
    return;
end;

if ~(isnumeric(skip) && length(skip)==1 && skip==fix(skip) && skip >= 0)
    disp('The fourth input (skip) to BZ2_fread should be a non-negative integer');
    return;
end;

if ~ischar(machineformat)
    disp('The fifth argument to BZ2_fread should be a machine format string.');
    return;
end;

%% check if file is open for reading
if BZ2_fid>length(gbl_BZ2_FID_INFO) || isempty(gbl_BZ2_FID_INFO(BZ2_fid).bzfid) || ~strcmp(gbl_BZ2_FID_INFO(BZ2_fid).mode,'r')
    disp('In BZ2_ftell:  The BZ2_fid entered does not correspond to a file open for reading');
    return;
end;
...
if length(precision_tokens)<1
    disp('Invalid precision string in BZ2_fread -- see fread help, and use matlab precision names');
end;
...
%% parse count
if count(end)==Inf
    filesize = BZ2_filesize(BZ2_fopen(BZ2_fid));
    count_to_read = fix((filesize-BZ2_ftell(BZ2_fid))/bytes_per_element);
...
swapbytes = gbl_BZ2_FID_INFO(BZ2_fid).swapbytes;
if ~isempty(machineformat)
    ...
    elseif strcmp(machineformat,'l') || strcmp(machineformat,'ieee-le')
        ...
        disp('machineformat input not understood, using format specified at BZ2_fopen');
    end;
end;

%% read data
numtoread = prod(count) + skip * fix(prod(count)/skip_interval);
startpos = double(gbl_BZ2_FID_INFO(BZ2_fid).bzfid(4));
[gbl_BZ2_FID_INFO(BZ2_fid).bzfid data] = BZ2Read('read',gbl_BZ2_FID_INFO(BZ2_fid).bzfid,uint32(numtoread),precision_tokens{1}{3},swapbytes);
bytesread = double(gbl_BZ2_FID_INFO(BZ2_fid).bzfid(4))-startpos;
