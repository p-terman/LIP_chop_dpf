% function xmlstring = MakeXMLString(xmlstruct,indentlevel)
%
...
function [xmlstring success] = MakeXMLString(xmlstruct,indentlevel)
    ...
    if isstruct(xmlstruct.(fields{f}))
        if prod(size(xmlstruct.(fields{f}))) == length(xmlstruct.(fields{f}))
            for s=1:length(xmlstruct.(fields{f}))
                ...
                [subxmlstring subsuccess] = MakeXMLString(xmlstruct.(fields{f})(s),indentlevel+1);
                if subsuccess
