function status = PulseQuantities_MinimumSet_split_sim(filename_evt,data_path_evt,filename_rq,data_path_rq,data_processing_xml_path,iq_xml_path)
%This just loads the evt file from .mat instead of binary. This program can
%be eliminated if I ever get around to writing a binary evt writer
...
%       20190101 -PAT modified this file for select reprocessing of LIPs
%                   basically just allow to load evt from mat file
% RQ versions:
%
%
%
...
%this is the only change for the split sim version. 
full_evt_file_name = [data_path_evt filesep filename_evt '.mat'];
holder = load(full_evt_file_name);
name = fieldnames(holder);
name = name{1};
event_struct = holder.(name);
    ...
    if isfield(cvt_struct(evt),'sumpod_data_phe_per_sample') && cvt_struct(evt).empty == 0
        ...
        for pp = 1:loop_max
            ...
            if sum(pulse_cut)==0 % if-then statment added 160301 by PAT this excludes pulses that have no pulse data in a time period. 
                dp.pulse_start_samples(pp,evt)=999999;
                dp.pulse_end_samples(pp,evt)=999999;
                continue             
            end
          ...
          if dp.amis1_fraction(pp,evt) > 10 || dp.amis1_fraction(pp,evt) < -10
			...
			% Pre and post trigger baseline rqs. Corrected to use POD array instead of peak array - AL - 2013/08/02
                        if length(cvt_struct(evt).ch(ch).pod_data_phe_per_sample) >= 24 % added to fix occational crash - JRV - 2013-03-04
                            dp.peak_detect_pretrigger_mean_phe(pp,ch,evt) = mean(cvt_struct(evt).ch(ch).pod_data_phe_per_sample(1:24));
                            dp.peak_detect_pretrigger_std_phe(pp,ch,evt)  = std(cvt_struct(evt).ch(ch).pod_data_phe_per_sample(1:24));
                        end
                        
                        if length(cvt_struct(evt).ch(ch).pod_data_phe_per_sample) >= 31 % added to fix occational crash - JRV - 2013-03-04
                            dp.peak_detect_posttrigger_mean_phe(pp,ch,evt) = mean(cvt_struct(evt).ch(ch).pod_data_phe_per_sample((end-31+1):end));
                            dp.peak_detect_posttrigger_std_phe(pp,ch,evt) = std(cvt_struct(evt).ch(ch).pod_data_phe_per_sample((end-31+1):end));
                        end
