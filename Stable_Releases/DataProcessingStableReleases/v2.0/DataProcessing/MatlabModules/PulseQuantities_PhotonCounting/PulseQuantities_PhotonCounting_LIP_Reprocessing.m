function status = PulseQuantities_PhotonCounting_LIP_Reprocessing(filename_evt,data_path_evt,filename_rq,data_path_rq,data_processing_xml_path,iq_xml_path)
%
...
% 20180423 PAT modified for reprocessing only candidates for LIP analysis
%RQ versions:
%
%
%
...
N = length(dp.event_number); %PAT changed from cvt_struct
...
for i = 1:length(dp.event_number)% changed from 1:N - PAT, now only does ones that need reprocessing
    ...
    if isfield(cvt_struct(evt),'sumpod_data_phe_per_sample') && cvt_struct(evt).empty == 0
        loop_max = min([sum(isfinite(dp.index_kept_sumpods(:,i))) max_num_pulses]);
                    ...
                    if ~isempty(peak_cut) && sum(peak_cut) > 0
                        ...
                        if ~isempty(peak_above_threshold)
                            if numel(peak_data_height) > 2
                                dp.peak_width_samples(pp,ch,i) = max(peak_above_threshold)-min(peak_above_threshold)+1;
                                ...
                                dp.spike_count(pp,ch,i) = sum(Variacao(2:end)<0 & Variacao(1:end-1)>=0);
                            end
                        end
