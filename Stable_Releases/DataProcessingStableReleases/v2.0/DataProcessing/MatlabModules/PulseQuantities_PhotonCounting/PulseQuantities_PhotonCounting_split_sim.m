function status = PulseQuantities_PhotonCounting_split_sim(filename_evt,data_path_evt,filename_rq,data_path_rq,data_processing_xml_path,iq_xml_path)
%
...
%this should flip the one event rqs. PAT 190226
if length(dp.luxstamp_samples) == 1
    fields = fieldnames(dp);
    for jj= 1: length(fields)
        current_field = fields{jj};

        if ndims(dp.(current_field)) == 2

            if size(dp.(current_field)) == [1 max_num_pulses]               
                dp.(current_field) = dp.(current_field)';
            end
        end
    end
end
