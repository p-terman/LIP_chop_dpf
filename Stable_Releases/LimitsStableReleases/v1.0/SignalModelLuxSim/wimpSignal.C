...
double wimpSignal(Double_t mWimp, Double_t minRawS2_phe=200., Int_t minEnergy_eV=3000, Int_t minS1_phe=2, Int_t maxS1_phe=40){
...
//   eframe->Draw();
  ...
  TFile wimpEventDensityFile(TString::Format("wimpDensityFile_%05iGeV_S1_%uto%uphe.root",int(mWimp),minS1_phe,maxS1_phe),"recreate");
  ...
  for(int e_eV=minEnergy_eV;e_eV<(1000.*maxEnergy_keV+1);e_eV+=250){ // you might want to change this (low energy cutoff)
    ...
    events->Draw("log10(S2b/S1c):S1c>>+wimpEventDensity",TString::Format("(S2r>%.1f&&golden[0])*%G",minRawS2_phe,weighting),"goff");
...
//   wimpEventDensity_1->Draw("same");
  ...
  cWimps->Print(TString::Format("wimpDensity_%05iGeV_S1_%uto%uphe.pdf",int(mWimp),minS1_phe,maxS1_phe));
